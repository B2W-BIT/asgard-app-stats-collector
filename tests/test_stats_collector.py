...
class StatsCollectorTest(asynctest.TestCase):
    def setUp(self):
        self.maxDiff = None
        self.logger = asynctest.Mock(spec=JsonLogger, exception=CoroutineMock())

    async def test_extract_app_name(self):
        self.assertEqual(
            "/afiliados/infra/reverse-proxy/certbot-public-afiliados.com.br",
            extract_app_name(
                {
                    "executor_id": "afiliados_infra_reverse-proxy_certbot-public-afiliados.com.br.8f91eac8-e2b6-11e8-ad9b-de0cd2503c7d"
                }
            ),
        )
        self.assertEqual(
            "/afiliados/infra/reverse-proxy/certbot-public",
            extract_app_name(
                {
                    "executor_id": "afiliados_infra_reverse-proxy_certbot-public.8f91eac8-e2b6-11e8-ad9b-de0cd2503c7d"
                }
            ),
        )
    ...
    async def test_remove_empty_stats_from_final_list(self):
        ...
        with aioresponses() as m, mock.patch.object(
            conf,
            "cache",
            CoroutineMock(get=CoroutineMock(), set=CoroutineMock()),
            create=True,
        ) as redis_mock:
            ...
            m.get(
                ...
                payload=self.slave_statistics_multiple_tasks,
            )

            slave_statistics = await get_slave_statistics(
                "10.0.111.32:5051", self.logger
            )
            self.assertEqual(1, len(slave_statistics))
            self.assertEqual(
                "infra_stress.0741cf07-dde6-11e8-a6bb-0242ac120020",
                self.slave_statistics_multiple_tasks[0]["executor_id"],
            )
    ...
    async def test_add_task_to_cache_on_first_fetch(self):
        ...
        with mock.patch.object(
            ...
            self.assertIsNone(
                await build_statistic_for_response("10.168.200.55", task_info)
            )
            redis_mock.get.assert_awaited_with(task_info["executor_id"])
            redis_mock.set.assert_awaited_with(
                task_info["executor_id"], json.dumps(task_info)
            )
    ...
    async def test_calculate_metrics_if_task_already_in_cache(self):
        ...
        with mock.patch.object(
            ...
            CoroutineMock(get=CoroutineMock(), set=CoroutineMock()),
            create=True,
        ) as redis_mock, mock.patch.multiple(
            conf, STATS_COLLECTOR_INCLUDE_RAW_METRICS=True
        ):
            redis_mock.get.return_value = json.dumps(task_info_before)
            calculated_metrics = await build_statistic_for_response(
                "10.168.200.55", task_info_now
            )
            redis_mock.get.assert_awaited_with(task_info_now["executor_id"])
            redis_mock.set.assert_awaited_with(
                task_info_now["executor_id"], json.dumps(task_info_now)
            )
    ...
    async def test_calculate_metrics_for_task_without_cfs_enabled(self):
        ...
        task_info_before = self.multuple_reads_monitor_statistics_cfs_off[
            "before"
        ]
        ...
        with mock.patch.object(
            ...
            expected_results = {
                ...
                **self.multuple_reads_monitor_statistics_cfs_off[
                    "expected_result"
                ],
            }
            self.assertDictEqual(expected_results, calculated_metrics)
    ...
    async def test_logs_exception_for_each_error_on_calculate_metrics(self):
        ...
        """
        with aioresponses() as resp, mock.patch.object(
            status_collector, "build_statistic_for_response"
        ) as build_statistic_for_response_mock:
            build_statistic_for_response_mock.side_effect = [
                {"taskname": "/infra/name"},
                Exception("Error"),
            ]
            resp.get(
                "http://10.0.1.1:5051/monitor/statistics.json", payload=[{}, {}]
            )
    ...
    async def test_logs_traceback_if_slave_fetch_fails(self):
        ...
        with aioresponses() as resp:
            resp.get(
                f"http://{slave_ip}/monitor/statistics.json",
                exception=aiohttp.client_exceptions.ClientConnectionError(
                    f"Error connecting to {slave_ip}"
                ),
            )
            with self.assertRaises(Exception):
                expected_result = await get_slave_statistics(
                    "10.0.1.1:5051", self.logger
                )
    ...
    async def test_decimal_com_arredondamento_correto(self):
        ...
        task_info_before = self.cpu_limit_valor_com_arredondamento_incorreto[
            "before"
        ]
